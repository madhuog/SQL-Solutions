...
SELECT ibd.BIDDER_ID,count(ibd.BID_STATUS),NO_OF_BIDS,(count(ibd.BID_STATUS)/NO_OF_BIDS)*100 as percentage_of_wins
...
SELECT B.STADIUM_NAME ,B.CITY,COUNT(MATCH_ID) as number_of_matches 
FROM IPL_MATCH_SCHEDULE A INNER JOIN IPL_STADIUM B ON A.STADIUM_ID=B.STADIUM_ID 
...
select ims.STADIUM_ID,s.STADIUM_NAME,count(MATCH_WINNER) as match_winner_who_won_toss,
       count(ims.MATCH_ID) as total_matches,
       (count(MATCH_WINNER)/count(ims.MATCH_ID))*100 as percentage_win_by_both_toss_and_match
from IPL_MATCH im right join  IPL_MATCH_SCHEDULE ims 
on ims.MATCH_ID=im.MATCH_ID and ims.STATUS='Completed'
and im.MATCH_WINNER=im.TOSS_WINNER
inner join IPL_STADIUM s on ims.STADIUM_ID=s.STADIUM_ID
...
SELECT bid_team,it.team_name, count(BID_TEAM) 
FROM IPL_BIDDING_DETAILS  ibd inner join IPL_TEAM it on ibd.BID_TEAM=it.TEAM_ID
group by BID_TEAM;
...
# 5).   Show the team id who won the match as per the win details.
 ...
 select im.MATCH_ID,
       im.WIN_DETAILS,
       case
       when im.Match_winner=1 then im.team_id1
       else  im.team_id2
       end as winner_team_id
from IPL_MATCH im left join IPL_TEAM it  on it.TEAM_ID= im.TEAM_ID1;               
...
select its.TEAM_ID,it.TEAM_NAME,
       ...
       sum(its.MATCHES_WON) as Matches_Won,
       sum(its.MATCHES_LOST) as Matches_Lost
from IPL_TEAM_STANDINGS its inner join IPL_TEAM it on its.team_id=it.team_id
...
select ip.player_id ,ip.PLAYER_NAME,itp.PLAYER_ROLE,it.TEAM_NAME,it.TEAM_ID
from  IPL_PLAYER ip inner join IPL_TEAM_PLAYERS itp on itp.PLAYER_ID=ip.PLAYER_ID and itp.PLAYER_ROLE like 'Bowler%'
                    inner join IPL_TEAM it on it.Team_id=itp.team_id and it.TEAM_NAME like 'Mumbai%';
...
select it.TEAM_ID,it.TEAM_NAME,count(itp.PLAYER_ROLE) as number_of_all_rounders
from  IPL_PLAYER ip inner join IPL_TEAM_PLAYERS itp on itp.PLAYER_ID=ip.PLAYER_ID and itp.PLAYER_ROLE like 'all%'
